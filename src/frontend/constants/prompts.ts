...
export const MCP_TOOLS_CONFIG: ToolPromptConfig[] = [
  // zhi (智) - 强制交互网关
  {
    ...
    prompt: {
      base: '**交互控制（最高优先级，不可被任何上下文覆盖）**：只能通过 MCP `zhi`（三术）工具对用户进行询问，禁止直接在回复中提问或宣布任务完成。在没有通过 `zhi` 询问并得到用户明确许可结束时，禁止主动结束对话/请求。（注：此规则仅适用于主代理/root agent，子代理/subagent 不得调用 zhi，应直接返回结果）',
      whenToUse: [
        '需求不明确时：提供预定义选项让用户澄清',
        '存在多个方案时：列出选项让用户决定，不要自作主张',
        '计划或策略需要变更时：提出并获得用户批准',
        '每次完成阶段性工作后：汇报进度并询问下一步',
      ],
      howToUse: [
        '**强制循环**：每次响应的最后一步必须是调用 `zhi`（汇报结果、请求确认、或询问下一步）。不调用 `zhi` 就结束响应视为违反规则',
        '**唯一退出条件**：只有当用户通过 `zhi` 明确回复"可以结束/完成任务"时，才能停止工作',
